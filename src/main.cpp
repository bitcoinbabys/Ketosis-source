...
bool fDevFee(int nHeight)
	{
	if (nHeight < 129600) return false;
	return (nHeight % 10080 < 1);}

int64_t nDevFee = 32850 * COIN;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    else if (pindexLast->nHeight < Params().StartPOS())
...
//            CBitcoinAddress address("KLWvqx1opVYU5o9E7ywRTznVM66qaSYjtH");
    	...
    	// check coinbase == miner reward (no devFee)
    	    if (vtx[0].GetValueOut() > nReward)
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfStake() && nHeight < Params().StartPOS())
